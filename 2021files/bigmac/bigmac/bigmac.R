...
chart <- function(country){
  ...
  data %>%
    ...
    theme(plot.background = element_rect(fill = NA, color = NA),
          ...
          plot.title = element_text(family = "heebo", size = 12))
  
}

centre <- map_data("world") %>% tbl_df %>% 
  filter(region %in% c("Norway", "Switzerland", "South Africa", "Argentina", "China", "Russia",
...
final <- map + 
  ...
  plot_annotation(
    ...
    theme = theme(plot.caption = element_text(family = "heebo", size = 10, color = "#183170"),
                  plot.title = element_text(family = "oswald", hjust = 0.5, size = 28, face = "bold", color = "#183170", margin = margin(5,0,0,0)))
  )

ggsave(final,file="big-index.png",device = NULL,path = NULL,width = 25,height = 15,
       units = c("in"),dpi = 300)

ggsave(map,file="mpg.png",device = NULL,path = NULL,width = 25,height = 15,
       units = c("in"),dpi = 300)
